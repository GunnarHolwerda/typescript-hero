...
export class TsResourceParser {
    ...
    constructor(logger: Logger) {
        this.logger = logger.createSpecificLogger('TsResourceParser');
    }
     ...
     * 
     * @param {string} source
     * @returns {Promise<TsFile | undefined>}
     * 
     * @memberOf TsResourceParser
     */
    public async parseSource(source: string): Promise<TsFile | undefined> {
        try {
     ...
     * @memberOf TsResourceParser
     */
    public async parseFile(file: string): Promise<TsFile> {
     ...
     * @memberOf TsResourceParser
     */
    public async parseFiles(filePathes: string[]): Promise<TsFile[] | undefined> {
        try {
     ...
     * @memberOf TsResourceParser
     */
    private parse(tsResource: TsResource, node: Node): void {
    ...
    private parseExport(tsResource: TsResource, node: ExportDeclaration | ExportAssignment): void {
        if (isExportDeclaration(node)) {
            ...
            let tsExport = node as ExportAssignment;
            let literal = tsExport.expression as Identifier;
            if (tsExport.isExportEquals) {
                tsResource.exports.push(
                    new TsAssignedExport(tsExport.getStart(), tsExport.getEnd(), literal.text, tsResource)
                );
            } else {
    ...
    private parseCtorParams(
        ...
        node.parameters.forEach(o => {
            if (isIdentifier(o.name)) {
                ...
                ctor.parameters = ctor.parameters.concat(elements.map<any>((bind: BindingElement) => {
        ...
        return node.parameters.reduce((all: TshParameterDeclaration[], cur: ParameterDeclaration) => {
            if (isIdentifier(cur.name)) {
                ...
                let identifiers = cur.name as ObjectBindingPattern | ArrayBindingPattern,
                    elements = [...identifiers.elements];
                all = all.concat(elements.map<any>((o: BindingElement) => {
                    if (isIdentifier(o.name)) {
                        return new TshParameterDeclaration(
