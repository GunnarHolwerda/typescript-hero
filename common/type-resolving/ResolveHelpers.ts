...
//import { ImportLocation } from '../ts-parsing';
 ...
 * 
 * @export
 * @param {DeclarationInfo[]} declarations
 * @param {string} workspaceRoot Root path of the actually openend workspace
...
export function getDeclarationsFilteredByImports(
    ...
    for (let tsImport of imports) {
        let importedLib = getAbsolutLibraryName(tsImport.libraryName, workspaceRoot, documentPath);
 ...
 */
export function getAbsolutLibraryName(library: string, workspaceRoot: string, actualFilePath: string): string {
    if (!library.startsWith('.')) {
        return library;
    }

    return '/' + relative(workspaceRoot, normalize(join(parse(actualFilePath).dir, library))).replace(/[/]$/g, '');
}
